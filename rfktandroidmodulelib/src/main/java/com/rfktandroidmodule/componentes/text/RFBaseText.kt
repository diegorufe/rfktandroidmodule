...
/**
 * Clase base para textos
 */
open class RFBaseText @JvmOverloads constructor(
    context: Context,
    ...
    var defaultPadding: Int? = IRFKTComponentsConstants.PADDING_DEFECTO_TEXT;


    var defaultFontSize: Int? = IRFKTComponentsConstants.FONT_SIZE_DEFECTO_TEXT;


    var defaultTextColor: Int? = IRFKTComponentsConstants.FONT_COLOR_DEFECTO_TEXT;
    ...
    private fun initStyleSheetAtributtes(attrs: AttributeSet?) {
        val typedArray: TypedArray =
            this.context.theme.obtainStyledAttributes(attrs, R.styleable.RFBaseText, 0, 0);

        for (i in 0 until typedArray.indexCount) {
            when (typedArray.getIndex(i)) {
                R.styleable.RFBaseText_text_defaultBackColor -> this.defaultBackColor =
                    this.defaultBackColor?.let { typedArray.getInt(typedArray.getIndex(i), it) }

                R.styleable.RFBaseText_text_defaultBack -> this.defaultBackDrawable =
                    this.defaultBackDrawable?.let { typedArray.getInt(typedArray.getIndex(i), it) }

                R.styleable.RFBaseText_text_defaultFontSize -> this.defaultFontSize =
                    this.defaultFontSize?.let { typedArray.getInt(typedArray.getIndex(i), it) }

                R.styleable.RFBaseText_text_defaultPadding -> this.defaultPadding =
                    this.defaultPadding?.let {
                        typedArray.getDimension(typedArray.getIndex(i), it.toFloat())
                            .absoluteValue.toInt()
                    }

                R.styleable.RFBaseText_text_defaultTextColor -> this.defaultTextColor =
